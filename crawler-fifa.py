...
def getPlayerStats(id_player):
    ...
    height = int(re.search("[0-9]*(?=cm)", pInfo).group(0))
    weight = int(re.search("[0-9]*(?=kg)", pInfo).group(0))
...
for index, league in enumerate(leagues):
    ...
    with open("./leagues.json", 'w', encoding='utf-8') as fp:
...
with open("./fifadata.json", 'w', encoding='utf-8') as fp:
